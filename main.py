...
class Comment(db.Model):
    ...
    parent_posts_id = db.Column(db.Integer, db.ForeignKey("blog_posts.id"))
...
def load_user(user_id):
    return db.session.query(AddUser).get(int(user_id))
...
def register():
    ...
    already_user = db.session.query(AddUser).filter_by(email=email).first()
...
def login():
    ...
    if form.validate_on_submit():
        email = request.form.get("email")
        password = request.form.get("password")
        user = db.session.query(AddUser).filter_by(email=email).first()
        if not user:
            flash("This email address does not exist. Try again")
            return redirect(url_for("login"))
